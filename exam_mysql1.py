...
#테이블 생성(데이터 타입 : 공식 레퍼런스 참조 또는 검색 참조) #\줄바꿈 바로 다음줄과 이어진다
...
try:
    with conn.cursor() as c:
        #JSON to MySQL 삽입1
        with open('C:/Users/heesun_estsecurity/Desktop/autotest/inflearn/webcrawling/section5/users.json','r') as infile:
            r = json.load(infile)
            userData = []
            for user in r:
